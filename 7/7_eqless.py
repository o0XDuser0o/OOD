...
class BST:
    ...
    def findless(self,data):
        return self._findless(data,self.root)

    def _findless(self,data,root: Node): #กูคือฟังชั่นแม่
        if root == None:
            return 0
        size = 0
        size = size + self._findless(data,root.left)# กูคือฟังชั่นลูก
        if data >= root.data:
            size += 1
        size = size + self._findless(data,root.right)# กูคือฟังชั่นลูก
        return size
    
T = BST()
lit,data = input('Enter input: ').split("/")
for i in [int(i) for i in lit.split(" ")]:
    T.insert(i)
